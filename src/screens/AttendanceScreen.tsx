...
export default function AttendanceScreen() {
  ...
  const displayAddress =
    backendAddress ||
    currentAddress ||
    (currentCoords ? 'Resolving live address...' : activeWorkLocation.label);
  ...
  const updateLocationFromPosition = useCallback(
    async (
      ...
      // Geocode user's current coordinates immediately to show the actual current address
      try {
        const addrString = `${coords.latitude},${coords.longitude}`;
        const geocoded = await geocodeAddressAPI(addrString, authToken ?? '');
        if (isMounted.current) {
        ...
        console.warn('[AttendanceScreen] Immediate geocoding failed', e);
        if (isMounted.current) {
          const coordString = `${coords.latitude.toFixed(5)}, ${coords.longitude.toFixed(5)}`;
          setCurrentAddress(coordString);
          setBackendAddress(coordString);
        }
      }

      await runBackendLocationCheck(coords, forceBackendCheck);
    },
    [runBackendLocationCheck, authToken],
  );
