...
export default function HomeScreen() {
 ...
 const lastLocationCheckToken = useRef<string | null>(null);
 ...
 const runHomeLocationCheck = useCallback(async () => {
  const token = auth.token?.trim();
  if (!token || locationCheckInFlight.current) {
   return;
  }

  if (lastLocationCheckToken.current === token) {
   return;
  }
  ...
  try {
   ...
   if (permissionStatus !== RESULTS.GRANTED) {
    lastLocationCheckToken.current = token;
    return;
   }
    ...
    Geolocation.getCurrentPosition(
     async position => {
      try {
       ...
       console.warn('[HomeLocation] get-check-location failed', error);
      } finally {
       resolve();
      }
     },
     error => {
      console.warn('[HomeLocation] Unable to get current position', error);
      resolve();
     },
     {
   ...
   console.warn('[HomeLocation] Location check failed', error);
  } finally {
 ...
 useEffect(() => {
  runHomeLocationCheck();
 }, [runHomeLocationCheck]);

 const handleOpenBillableTravel = useCallback(() => {
 ...
 const handleIgnoreLocationReminder = useCallback(() => {
  ...
  syncBackendLocationReminderNotification(null).catch(error =>
   console.warn('[HomeLocation] Unable to dismiss location reminder', error),
  );
 }, []);

 useEffect(() => {
  const handlerMap: Partial<Record<NotificationActionId, () => void>> = {
   'open-map': handleOpenBillableTravel,
   'bill-time': handleOpenBillableTravel,
   'check-in': () => navigation.navigate('Attendance'),
  };
 ...
 }, [handleOpenBillableTravel, navigation]);

 const backendLocationBanner = useMemo(() => {
  ...
  const openMapAction = actions.find(action => action.id === 'OPEN_MAP');
     ...
     ? [
        {
         ...
         onPress: handleOpenBillableTravel,
        },
       ]
     : []),
    {label: 'Ignore', onPress: handleIgnoreLocationReminder},
   ],
  };
 }, [
  backendLocationReminder,
  handleIgnoreLocationReminder,
  handleOpenBillableTravel,
 ]);
  ...
  <SafeAreaView style={styles.safe}>
   ...
   <ScrollView showsVerticalScrollIndicator={false}>
    {backendLocationBanner ? (
     <NotificationBanner
      variant="info"
      title={backendLocationBanner.title}
      description={backendLocationBanner.description}
      icon="navigation"
      actions={backendLocationBanner.actions}
      style={styles.locationReminderBanner}
     />
    ) : null}
...
const buildHomeProfile = (
 ...
 return {
  alertText: hasPendingReport ? eodMessage : 'Pending E.O.D Report',
